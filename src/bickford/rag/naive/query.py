...
def call_api(
    ...
    with open("query.log", "a") as f:
        f.write(f"prompt: {type(prompt)} {prompt}\n")
        f.write(f"options: {type(options)} {options}\n")
        f.write(f"context: {type(context)} {context}\n")
        f.write("--------------------------------\n")
    ...
    answer, retrieved_files = generate_response(retriever, question)

    # Store retrieved files in metadata for promptfoo assertions
    return {
        "output": answer,
        "metadata": {
